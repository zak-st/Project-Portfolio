...
int* level2CurrentPlayerOffset = 0;
int level2CurrPlayerFrame = 0;
float* playerTime;

/*ghost animation variables*/
TextureOffset ghostOffsets[2];
int* level2CurrentGhostOffset = 0;
int level2CurrGhostFrame = 0;
float* ghostTime;

/*enemy2 animation variables*/
TextureOffset enemy2Offsets[3];
int* level2CurrentEnemy2Offset = 0;
int level2CurrEnemy2Frame = 0;
float* enemy2Time;

/*enemy3 animation variables*/
TextureOffset enemy3Offsets[2];
int* level2CurrentEnemy3Offset = 0;
int level2CurrEnemy3Frame = 0;
float* enemy3Time;

/*enemy4 animation variables*/
TextureOffset enemy4Offsets[2];
int* level2CurrentEnemy4Offset = 0;
int level2CurrEnemy4Frame = 0;
float* enemy4Time;

signed long level2MousePosX, level2MousePosY;
float level2MouseWorldPosX, level2MouseWorldPosY;
float level2CamX, level2CamY;

int level2LastPlayerDir = 0;

int level2DustCounter = 60;

int level2NumLives = 9; /*number to be decrimented*/
const int level2MaxLives = 9; /*constant max*/
...
/*metrics data*/
unsigned int level2Start_frame = 0;          /*frame on which gameplay was started*/
unsigned int level2Ghosts_killed = 0;        /*number of ghosts killed*/
unsigned int level2Checkpoints_lit = 0;      /*number of checkpoints hit*/
float level2Distance_run = 0;                /*total distance run by player*/
...
void Level2Load()
{
	...
	/*load map file*/
	map_create("Assets/level2_map.txt");
...
void Level2Init()
{
	...
	level2Start_frame = AEFrameRateControllerGetFrameCount();
	...
	/*init player*/
	level2Player = (Player*)(malloc(sizeof(Player)));
	create_player(level2Player);
...
void Level2Update(float dt)
{
	...
	AEInputGetCursorPosition(&level2MousePosX, &level2MousePosY);
	AEGfxConvertScreenCoordinatesToWorld(level2MousePosX, level2MousePosY, &level2MouseWorldPosX, &level2MouseWorldPosY);

	/*throw hook*/
	if (AEInputCheckTriggered(RI_MOUSE_LEFT_BUTTON_DOWN) && hookTrigger == 1)
	{
		playSoundAdvanced("Assets/Sounds/hook_shoot.wav", 0.4f);
		hook_throw(player, level2MouseWorldPosX, level2MouseWorldPosY);
	}
	...
	if (object_list)
	{
		...
		while (i < num_objects)
		{
			...
			case PLAYER:
				...
				{
					player_move(curr, level2Player, dt);
				}
				...
				level2Distance_run += (player->vel.x > 0 ? player->vel.x : -player->vel.x);
				if (curr->coll_flag) snap(curr, curr->coll_flag);
				//create dust particles
				if (level2DustCounter > 15)
				{
				...
				while (j < num_objects)
				{
					...
					if ((curr2->type != PLAYER) && bounding_box_check(curr, curr2))
					{
						...
						case FINALCHECK:
						{
							/*print metrics*/
							level2Checkpoints_lit++;
							AESysPrintf("Time elapsed since start of level: %f seconds\n", (float)(AEFrameRateControllerGetFrameCount() - level2Start_frame) * AEFrameRateControllerGetFrameTime());
							AESysPrintf("Ghosts killed: %d\nCheckpoints touched: %d\n", level2Ghosts_killed, level2Checkpoints_lit);
	...
	if (object_list)
	{
		...
		while (i < num_objects)
		{
			...
			case PLAYER:
				if (curr->isAlive == 1)
				{
					...
					if (player_moving_left() != 0)
					{
						...
						animateFrames(&level2CurrentPlayerOffset, &playerTime, 1, dt);
						checkEndFrames(&level2CurrentPlayerOffset, 4);
						level2CurrPlayerFrame = level2CurrentPlayerOffset;
						AEGfxTextureSet(playerWalkTexture, playerOffsets[level2CurrPlayerFrame].mX, playerOffsets[level2CurrPlayerFrame].mY);

						level2LastPlayerDir = 1;
						isWalking = 1;
					}
					...
					if (!AEInputCheckCurr("A") && level2LastPlayerDir == 1 && isWalking == 0)
					{
						//flip the sprite
						AEGfxTextureSet(playerIdleTexture, 0.0f, 0.0f);
						AEGfxSetFullTransform(curr->pos.x, curr->pos.y, 0.0f, -(curr->width * tile_dim), curr->height * tile_dim);
			...
			case ENEMY_1:
				...
				animateFrames(&level2CurrentGhostOffset, &ghostTime, 2, dt);
				checkEndFrames(&level2CurrentGhostOffset, 2);
				level2CurrGhostFrame = level2CurrentGhostOffset;
				AEGfxTextureSet(ghostTexture, ghostOffsets[level2CurrGhostFrame].mX, ghostOffsets[level2CurrGhostFrame].mY);
				break;
			case ENEMY_2:
				if (curr->vel.y != -5.0f)
				{
				...
				animateFrames(&level2CurrentEnemy2Offset, &enemy2Time, 2, dt);
				checkEndFrames(&level2CurrentEnemy2Offset, 2);
				level2CurrEnemy2Frame = level2CurrentEnemy2Offset;
				AEGfxTextureSet(enemy2Texture, enemy2Offsets[level2CurrEnemy2Frame].mX, enemy2Offsets[level2CurrEnemy2Frame].mY);
				break;
			case ENEMY_3:
				if (curr->vel.x != -2.5f)
				{
				...
				animateFrames(&level2CurrentEnemy3Offset, &enemy3Time, 2, dt);
				checkEndFrames(&level2CurrentEnemy3Offset, 2);
				level2CurrEnemy3Frame = level2CurrentEnemy3Offset;
				AEGfxTextureSet(enemy3Texture, enemy3Offsets[level2CurrEnemy3Frame].mX, enemy3Offsets[level2CurrEnemy3Frame].mY);
				break;
			case ENEMY_4:
				if (curr->vel.y != -2.5f)
				{
				...
				animateFrames(&level2CurrentEnemy4Offset, &enemy4Time, 2, dt);
				checkEndFrames(&level2CurrentEnemy4Offset, 2);
				level2CurrEnemy4Frame = level2CurrentEnemy4Offset;
				AEGfxTextureSet(enemy4Texture, enemy4Offsets[level2CurrEnemy4Frame].mX, enemy4Offsets[level2CurrEnemy4Frame].mY);
				break;
			case CHECKPOINT:
				if (curr->isAlive == 1)
				{
	...
	LivesDraw(level2MaxLives, level2NumLives, playerHeartOn, pMesh2);
...
void Level2Shutdown()
{
	/*reset metrics data*/
	level2Start_frame = 0;
	level2Ghosts_killed = 0;
	level2Checkpoints_lit = 0;
	level2Distance_run = 0;
