...
int currPlayerFrame = 0;
float* playerTime;

/*ghost animation variables*/
TextureOffset ghostOffsets[2];
int* currentGhostOffset = 0;
int currGhostFrame = 0;
float* ghostTime;

/*enemy2 animation variables*/
TextureOffset enemy2Offsets[3];
int* currentEnemy2Offset = 0;
int currEnemy2Frame = 0;
float* enemy2Time;

/*enemy3 animation variables*/
TextureOffset enemy3Offsets[2];
int* currentEnemy3Offset = 0;
int currEnemy3Frame = 0;
float* enemy3Time;

/*enemy4 animation variables*/
TextureOffset enemy4Offsets[2];
int* currentEnemy4Offset = 0;
int currEnemy4Frame = 0;
float* enemy4Time;

signed long mousePosX, mousePosY;
float mouseWorldPosX, mouseWorldPosY;
float camX, camY;

int lastPlayerDir = 0;

int demoDustCounter = 60;

int numLives = 9; /*number to be decrimented*/
const int maxLives = 9; /*constant max*/

Player* demoPlayer;
static char* debug_filename;

static float bgMusicTimer;
static float catSoundTimer;

/*metrics data*/
unsigned int start_frame = 0;          /*frame on which gameplay was started*/
unsigned int ghosts_killed = 0;        /*number of ghosts killed*/
unsigned int checkpoints_lit = 0;      /*number of checkpoints hit*/
float distance_run = 0;                /*total distance run by player*/
...
void DemoLoad()
{
	map_create("Assets/demo_map.txt");
...
void DemoInit()
{
	...
	catSoundTimer = 500.0f;

	start_frame = AEFrameRateControllerGetFrameCount();
	...
	/*init player*/
	demoPlayer = (Player*)(malloc(sizeof(Player)));
	create_player(demoPlayer);
...
void DemoUpdate(float dt)
{
	...
	AEGfxConvertScreenCoordinatesToWorld(mousePosX, mousePosY, &mouseWorldPosX, &mouseWorldPosY);

	/*throw hook*/
	if (AEInputCheckTriggered(RI_MOUSE_LEFT_BUTTON_DOWN) && hookTrigger == 1)
	{
		playSoundAdvanced("Assets/Sounds/hook_shoot.wav", 0.4f);
		hook_throw(player, mouseWorldPosX, mouseWorldPosY);
	}
	...
	if (object_list)
	{
		...
		while (i < num_objects)
		{
			...
			case PLAYER:
				...
				{
					player_move(curr, demoPlayer, dt);
				}
				...
				distance_run += (player->vel.x > 0 ? player->vel.x : -player->vel.x);
				if (curr->coll_flag) snap(curr, curr->coll_flag);
				//create dust particles
				if (demoDustCounter > 15)
				{
				...
				/*create dust particles if jumping*/
				if(player->vel.y > 0 && player->coll_flag & 8) 
					ParticleCreate(player);
				...
				while (j < num_objects)
				{
					...
					if ((curr2->type != PLAYER) && bounding_box_check(curr, curr2))
					{
						...
						case FINALCHECK: 
						{
							...
							/*print metrics*/
							checkpoints_lit++;
							AESysPrintf("Time elapsed since start of level: %f seconds\n", (float)(AEFrameRateControllerGetFrameCount() - start_frame) * AEFrameRateControllerGetFrameTime());
							AESysPrintf("Ghosts killed: %d\nCheckpoints touched: %d\n", ghosts_killed, checkpoints_lit);
	...
	if (object_list)
	{
		...
		while (i < num_objects)
		{
			...
			case PLAYER: 
				if (curr->isAlive == 1)
				{
					...
					if (player_moving_left() != 0)
					{
						...
						animateFrames(&currentPlayerOffset, &playerTime, 1, dt);
						checkEndFrames(&currentPlayerOffset, 4);
						currPlayerFrame = currentPlayerOffset;
						AEGfxTextureSet(playerWalkTexture, playerOffsets[currPlayerFrame].mX, playerOffsets[currPlayerFrame].mY);
			...
			case ENEMY_1: 
				...
				animateFrames(&currentGhostOffset, &ghostTime, 2, dt);
				checkEndFrames(&currentGhostOffset, 2);
				currGhostFrame = currentGhostOffset;
				AEGfxTextureSet(ghostTexture, ghostOffsets[currGhostFrame].mX, ghostOffsets[currGhostFrame].mY);
				break;
			case ENEMY_2: 
				if (curr->vel.y != -5.0f)
				{
				...
				animateFrames(&currentEnemy2Offset, &enemy2Time, 2, dt);
				checkEndFrames(&currentEnemy2Offset, 2);
				currEnemy2Frame = currentEnemy2Offset;
				AEGfxTextureSet(enemy2Texture, enemy2Offsets[currEnemy2Frame].mX, enemy2Offsets[currEnemy2Frame].mY);
				break;
			case ENEMY_3: 
				if (curr->vel.x != -2.5f)
				{
				...
				animateFrames(&currentEnemy3Offset, &enemy3Time, 2, dt);
				checkEndFrames(&currentEnemy3Offset, 2);
				currEnemy3Frame = currentEnemy3Offset;
				AEGfxTextureSet(enemy3Texture, enemy3Offsets[currEnemy3Frame].mX, enemy3Offsets[currEnemy3Frame].mY);
				break;
			case ENEMY_4:
				if (curr->vel.y != -2.5f)
				{
				...
				animateFrames(&currentEnemy4Offset, &enemy4Time, 2, dt);
				checkEndFrames(&currentEnemy4Offset, 2);
				currEnemy4Frame = currentEnemy4Offset;
				AEGfxTextureSet(enemy4Texture, enemy4Offsets[currEnemy4Frame].mX, enemy4Offsets[currEnemy4Frame].mY);
				break;
			case CHECKPOINT: 
				if (curr->isAlive == 1)
				{
	...
	LivesDraw(maxLives, numLives, playerHeartOn, pMesh2);
